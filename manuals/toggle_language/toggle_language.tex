...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
        \item 翻译新建数据块的名称按钮：启用/禁用新建数据块名称的翻译功能\footnote{v1.1之后版本不再是强制接管该功能。虽然可以通过用户偏好设置来设置一个不同于插件的设置值，但在使用插件的该功能或者语言切换按钮后，插件会对该项改写修正为插件的设置值。}，默认值为禁用，如图\myref{插件的翻译名称选项}所示，其对应在偏好设置中的选项如图\myref{原始的翻译名称选项}所示。在非英语界面环境中，启用该功能可使blender新建数据块的名称为当前语言，如图\myref{翻译名称的效果}所示，新建平面的名称为「平面」，而非「plane」。
...
\begin{figure}[h!]
    ...
    \quad
    \begin{minipage}[t]{0.47\textwidth}
        \includegraphics[scale=0.18]{translate_name_option_addon}
...
\begin{itemize}
    \item 禁用路径设置：禁用路径参数设置（默认不启用，即不禁用）。
    \item 禁用主题设置：禁用主题参数设置（默认不启用，即不禁用）。
...
\begin{figure}[h!]
    \includegraphics[scale=0.29]{settings_for_loading_my_settings}
